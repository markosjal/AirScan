...
if (($_GET['newext'] != 'pdf') && ($_GET['newext'] != 'jpg') && ($_GET['newext'] != 'png') && ($_GET['newext'] != 'gif') && ($_GET['newext'] != 'jpeg') && ($_GET['newext'] != 'webp'))
{
$newext='jpg';
}
else 
...
if (($_GET['newname'] == $basename) && ($_GET['newext'] == $ext))
{
$rotatedfile= $basename.$rotate.'.'.$newext;
$rotatedbasename=$basename;
$rotatedext=$ext;

}
elseif (($_GET['newname'] == '') || ($_GET['newname'] == NULL) || (!isset($_GET['newname'])))
{
$rotatedfile= $basename.$rotate.'.'.$newext;
$rotatedbasename=$basename;
$rotatedext=$ext;
...
{
$rotatedfile= $_GET['newname'].'.'.$newext;
$rotatedbasename=pathinfo($rotatedfile, PATHINFO_FILENAME);
$rotatedext=strtolower(pathinfo($rotatedfile, PATHINFO_EXTENSION));
$rotate='';
}
...
// delete image param
unset($params['newname']);
unset($params['newext']);
//unset($params['degrees']);
// change the print param
$params['image'] = $rotatedfile;
$params['rand'] = $rand;
// rebuild the query
...
$rotatecmd = $imagemagicklocation.' -rotate '.$degrees.' '.$previewimage.' '.$_SESSION["userpath"].$rotatedfile;
...
elseif (($_GET['return']=='index.php') && ($_SESSION['password']!='PAM') && (isset($_GET['degrees'])))
{
$refreshurl='<meta HTTP-EQUIV="REFRESH" content="'.$autocroprefresh.'; url='.$userpath.'index.php?rand='.$rand.'#'.$rotatedfile.'">';
//$refreshurl=$userpath.'index.php?rand='.$rand.'#'.$rotatedfile;
}

elseif (($_GET['return']=='pamindex.php') && ($_SESSION['password']=='PAM') && (isset($_GET['degrees'])))
{
$refreshurl='<meta HTTP-EQUIV="REFRESH" content="'.$autocroprefresh.'; url=pamindex.php?rand='.$rand.'#'.$rotatedfile.'">';
//$refreshurl='pamindex.php?rand='.$rand.'#'.$rotatedfile;
}


elseif (($_GET['return']=='airscan.php') && (isset($_GET['degrees'])))
{
...
if (!isset($_GET['degrees']))
{
	...
	echo '<center><p><span style="color:#666; font-weight:bold">'.$rotating.' '.$image.'</span></p>';
		...
		<input type="text" value="'.$rotatedbasename.$rotate.'" style="width: 160px; height: 15px; color:#666; background-color: #DDD; font-weight:bold" name="newname"></td><td><span style="color:#666; font-weight:bold">.</span></td><td>';
		...
		echo '>jpg</option><option value="png"';    		//start to png
			if (($_GET["newext"]=="png") || ($_GET["newext"]==NULL) || ($_GET["newext"]=="") || (!isset($_GET["newext"])))
			{
			echo ' selected '; 
			}
			else 
...
*/		//}	                                 //end from JPG
	...
	<input name="return" type="hidden" value="'.$_GET['return'].'">
        <input name="resolution" type="hidden" value="'.$_GET['resolution'].'">
